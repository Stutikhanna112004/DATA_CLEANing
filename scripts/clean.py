...
def main():
    ...
    # Write outputs
    users_clean.to_csv(OUT_DIR / "Social_Engine_Users_clean.csv", index=False)
    posts_clean.to_csv(OUT_DIR / "Social_Engine_Posts_clean.csv", index=False)
    users_clean.to_json(OUT_DIR / "Social_Engine_Users_clean.json", orient="records", indent=2)
    posts_clean.to_json(
        OUT_DIR / "Social_Engine_Posts_clean.json", orient="records", indent=2, date_format="iso"
    )

    summary = {"users": users_notes, "posts": posts_notes}
