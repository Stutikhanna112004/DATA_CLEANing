...
def main():
    df = pd.read_csv(PROC_DIR / "Social_Engine_clean.csv", parse_dates=["timestamp_parsed"])
    # user-level view: one row per user, for user-profile charts (follower count, location, language)
    users = df.drop_duplicates(subset="user_id")[["user_id", "location", "language", "account_created", "follower_count"]]
    posts = df
